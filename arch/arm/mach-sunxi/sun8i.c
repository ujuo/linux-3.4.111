...
static struct resource sunxi_nand_res[] = {
	{
		.start		= SUNXI_NANDFLASHC0_PBASE,
		.end		= SUNXI_NANDFLASHC0_PBASE + 0x1000,
		.flags		= IORESOURCE_MEM,
	},
	{
		.start		= 102,
...
static struct mtd_partition sunxi_mtd_partition[] = {
	{
		.name           = "bootloader",
		.offset         =   0,
		.size           = 512 * SZ_1K,
	...
	{
		.name		= "rootfs",
		.offset		= (5 * SZ_1M + 512 * SZ_1K),
		.size		= (8 * SZ_1M + 512 * SZ_1K),
	}, 
	{
		.name		= "userfs",
		.offset		= (16 * SZ_1M),
...
static struct platform_nand_chip sunxi_nand_chip = {
	.nr_partitions = sizeof(sunxi_mtd_partition) / sizeof(struct mtd_partition),
	.partitions = sunxi_mtd_partition,
};

static struct platform_nand_data sunxi_nand_data = {
	.chip = &sunxi_nand_chip,
};

static struct platform_device sunxi_nand_dev  = {
	.name = "sunxi_nand",
	.id = 0,
	.num_resources = 2,
	.resource = sunxi_nand_res,
	.dev	= {
		.platform_data	= &sunxi_nand_data,
	},
};
#endif
...
static struct i2c_board_info  gc2155_i2c_boardinfo[] = {
	{
		I2C_BOARD_INFO("gc2155", 0x78 >> 1),
		//.platform_data = &gc2155_plat,
	},
};

static struct i2c_board_info pcf8563_i2c_boardinfo[] = {
	{
...
//	pin_config_set(SUNXI_PINCTRL,SUNXI_PINCTRL_PIN_PB5,SUNXI_PINCFG_PACK(SUNXI_PINCTRL_TYPE_DAT,1));
	...
	pin_config_set(SUNXI_PINCTRL,SUNXI_PINCTRL_PIN_PL4,SUNXI_PINCFG_PACK(SUNXI_PINCTRL_TYPE_DRV,2));
	pin_config_set(SUNXI_PINCTRL,SUNXI_PINCTRL_PIN_PL4,SUNXI_PINCFG_PACK(SUNXI_PINCTRL_TYPE_DAT,1));

	
	printk("PL5 power on\n");
	pin_config_set(SUNXI_PINCTRL,SUNXI_PINCTRL_PIN_PL5,SUNXI_PINCFG_PACK(SUNXI_PINCTRL_TYPE_FUNC,1));
	pin_config_set(SUNXI_PINCTRL,SUNXI_PINCTRL_PIN_PL5,SUNXI_PINCFG_PACK(SUNXI_PINCTRL_TYPE_DRV,2));
	pin_config_set(SUNXI_PINCTRL,SUNXI_PINCTRL_PIN_PL5,SUNXI_PINCFG_PACK(SUNXI_PINCTRL_TYPE_DAT,1));



	printk("PL11 power on\n");
	pin_config_set(SUNXI_PINCTRL,SUNXI_PINCTRL_PIN_PL11,SUNXI_PINCFG_PACK(SUNXI_PINCTRL_TYPE_FUNC,1));
...
#endif
#endif
extern int __init script_init(void);
