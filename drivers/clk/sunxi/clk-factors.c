...
static long sunxi_clk_factors_round_rate(struct clk_hw *hw, unsigned long rate, unsigned long *prate)
{
    struct clk_factors_value factor_val;
	struct sunxi_clk_factors *factor = to_clk_factor(hw);
	printk("to_clk_factor %x hw %x\n", factor,hw);
    if(!factor->get_factors || !factor->calc_rate)
        return rate;
    if(prate != NULL)
    {
		printk("to_clk_factor rate %d prate %d\n", rate, *prate);
...
*/
   ...
   // factors->hw.init = &init;
    ...
    /* register the clock */
    clk = clk_register(dev, init_data->name,
    	init_data->priv_ops?(init_data->priv_ops):(&clk_factors_ops),
    	&factors->hw,
    	(char**)init_data->parent_names,
    	init_data->num_parents,
    	init_data->flags);
