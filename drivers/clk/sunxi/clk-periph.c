...
static int __sunxi_clk_periph_set_rate(struct clk_hw *hw, unsigned long rate)
{
    ...
    struct sunxi_clk_periph_div *divider = &periph->divider;
    unsigned long div, div_m = 0, div_n = 0;
    u64 tmp_rate =  __clk_get_rate(__clk_get_parent(hw->clk));
...
static int sunxi_clk_periph_set_rate(struct clk_hw *hw, unsigned long rate)
{
    unsigned long flags = 0;
    int ret = 0;
    struct sunxi_clk_periph *periph = to_clk_periph(hw);
//    unsigned long parent_rate = 0;
    ...
    {
        spin_lock_irqsave(periph->lock, flags);
        ret = __sunxi_clk_periph_set_rate(hw,rate);
        spin_unlock_irqrestore(periph->lock, flags);        
    }
    else
        ret = __sunxi_clk_periph_set_rate(hw,rate);
...
/*    init.name = name;
   ...
   // periph->hw.init = &init;
    ...
    clk = clk_register(NULL, name,
    	periph->priv_clkops?periph->priv_clkops:&sunxi_clk_periph_ops,
    	&periph->hw,
    	(char**)parent_names,
    	num_parents,
    	flags);
