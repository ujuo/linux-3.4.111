...
int parse_sensor_list_info(struct sensor_config_init *sensor_cfg_ini , char *pos)
{
	...
	sprintf(sensor_list_cfg, "/usr/etc/hawkview/sensor_list_cfg.ini");
...
void init_sensor_list_info(struct sensor_config_init *sensor_cfg_ini , char *pos)
{
	sensor_cfg_ini->used = 1;
	sensor_cfg_ini->csi_sel = 0;
	sensor_cfg_ini->device_sel = 0;
	sensor_cfg_ini->twi_id = 0;
	sensor_cfg_ini->power_settings_enable = 0;
	sensor_cfg_ini->detect_sensor_num = 0;
	sensor_cfg_ini->sub_power_str[0][0] = 0;
	sensor_cfg_ini->sub_power_vol[0] = 0;
	strcpy(sensor_cfg_ini->camera_inst[0].name,"test");
	sensor_cfg_ini->camera_inst[0].i2c_addr = 0;
	sensor_cfg_ini->camera_inst[0].sensor_type = 0;
	sensor_cfg_ini->camera_inst[0].stdby_mode = 0;
	sensor_cfg_ini->camera_inst[0].vflip = 0;
	sensor_cfg_ini->camera_inst[0].hflip = 0;
	sensor_cfg_ini->camera_inst[0].act_name[0] = 0;
	sensor_cfg_ini->camera_inst[0].act_i2c_addr = 0;	
	sensor_cfg_ini->camera_inst[0].isp_cfg_name[0] = 0;
}
...
#if defined(CONFIG_CSI_VFE_MOD)			
			parse_sensor_list_info(dev->ccm_cfg[i]->sensor_cfg_ini, dev->ccm_cfg[i]->sensor_pos);
#else
			/* Initialize sensor list from script bin. */
			init_sensor_list_info(dev->ccm_cfg[i]->sensor_cfg_ini, dev->ccm_cfg[i]->sensor_pos);
#endif
		}
