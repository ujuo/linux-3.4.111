...
static int sunxi_pctrl_list_groups(struct pinctrl_dev *pctrldev, unsigned selector)
{
	struct sunxi_pinctrl *pctl = pinctrl_dev_get_drvdata(pctrldev);
	if(selector > pctl->ngroups)
		return -EINVAL;
...
#endif /* CONFIG_OF */
	.list_groups		= sunxi_pctrl_list_groups,
...
static int sunxi_pmx_list_func(struct pinctrl_dev *pctrldev, unsigned func)
{
	struct sunxi_pinctrl *pctl = pinctrl_dev_get_drvdata(pctrldev);
	if (func >= pctl->nfunctions)
		return -EINVAL;
	return 0;
}
...
static struct pinmux_ops sunxi_pmx_ops = {
	.list_functions		= sunxi_pmx_list_func,
...
static int sunxi_pinctrl_probe(struct platform_device *pdev)
{
	...
	pctl->chip.base = 0;
	INIT_LIST_HEAD(&pctl->chip.pin_ranges);
