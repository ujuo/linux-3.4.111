...
{
	struct sunxi_nand_chip *sunxi_nand = mtd_to_sunxi_nand(mtd);
...
static void sunxi_nfc_hw_ecc_enable(struct mtd_info *mtd)
{
	struct sunxi_nand_chip *sunxi_nand = mtd_to_sunxi_nand(mtd);
	struct sunxi_nfc *nfc = to_sunxi_nfc(sunxi_nand->nand.controller);
	struct sunxi_nand_hw_ecc *data = sunxi_nand->nand.ecc.priv;
	u32 ecc_ctl;
...
#if !STRUCT_NANDCHIP_HAVE_ONFI_TIMING
static const struct nand_sdr_timings *onfi_async_timing_mode_to_sdr_timings(int mode)
{
...
static int sunxi_nand_chip_init(struct device *dev, struct sunxi_nfc *nfc, int index)
{
	const struct nand_sdr_timings *timings;
	struct sunxi_nand_chip *chip;
	struct sunxi_nand_plat_data *pdata = dev_get_platdata(dev);
	struct mtd_info *mtd;
	...
	for (i = 0; i < nsels; i++) {
		chip->sels[i].cs = nfc->pins[index].sels[i].cs;

		if (nfc->pins[index].sels[i].rb.type == CSRB_NATIVE) {
			chip->sels[i].rb.type = CSRB_NATIVE;
			chip->sels[i].rb.info.nativeid = nfc->pins[index].sels[i].rb.info.nativeid;
		} else 
		if (nfc->pins[index].sels[i].rb.type == CSRB_GPIO) {
			chip->sels[i].rb.type = CSRB_GPIO;
			tmp = chip->sels[i].rb.info.gpio = nfc->pins[index].sels[i].rb.info.gpio;
...
//	nand_set_flash_node(nand, np); // What this function doing?
	nand->select_chip = sunxi_nfc_select_chip;
	nand->cmd_ctrl = sunxi_nfc_cmd_ctrl;
	nand->read_buf = sunxi_nfc_read_buf;
	nand->write_buf = sunxi_nfc_write_buf;
	nand->read_byte = sunxi_nfc_read_byte;
	nand->options = 0; /* No options initialy. */
	...
	nand->ecc_strength_ds = 8;
#endif

	mtd = &chip->mtd; //nand_to_mtd(nand);
	mtd->dev.parent = dev;
	mtd->priv = nand; /* This should be done. */
	mtd->name = DRV_NAME; /* Don't know */
	mtd->owner = THIS_MODULE;  /* Don't know */

	ret = nand_scan_ident(mtd, 1, NULL); // nsels...
	if (ret) {
		dev_err(dev, "nand scan ident failed with %d\n", ret);
		return ret;
	}
	...
	/* set partitions for this device... */
...
int sunxi_nand_chips_init(struct device *dev, struct sunxi_nfc *nfc)
{
	int nchips = nfc->nres;
	int ret;
	int i;
	...
	for (i=0; i< nchips; i++) {
		ret = sunxi_nand_chip_init(dev, nfc, i);
		if (ret == 0) nands++;
		dev_err(dev, "sunxi nand chip init = %d\n", ret);
		if ((ret != 0) && (nands == 0))
			return ret;
	}
...
void sunxi_nand_chips_cleanup(struct sunxi_nfc *nfc)
{
	struct sunxi_nand_chip *chip;
