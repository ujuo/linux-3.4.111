/*
 * sound\soc\sunxi\audiocodec\v3_codec.c
...
#include "../../../drivers/pinctrl/pinctrl-sunxi.h"
#include <linux/pinctrl/consumer.h>
//#include "audiocodec/sunxi_codecdma.h"

#include "audiocodec/sun8iw8_sndcodec.h"
#include "v3-audio.h"

#define v3_codec_RATES  (SNDRV_PCM_RATE_8000_192000|SNDRV_PCM_RATE_KNOT)
#define v3_codec_FORMATS (SNDRV_PCM_FMTBIT_S8 | SNDRV_PCM_FMTBIT_S16_LE | \
...
static	bool version_v3_used       = true;
static	bool adcagc_used       = false;
static	bool adcdrc_used       = false;
static	bool dacdrc_used       = true;
static	bool adchpf_used       = false;
static	bool dachpf_used       = true;

static  int codec_recordsrc		= 0;
static	int codec_speaker_headset_en = 1;
static	int pa_vol 						= 0;
...
static struct v3_pcm_dma_params sunxi_pcm_pcm_stereo_out = {
	.name		= "audio_play",
	.dma_addr	= CODEC_BASSADDRESS + SUNXI_DAC_TXDATA,//send data address
};

static struct v3_pcm_dma_params sunxi_pcm_pcm_stereo_in = {
	.name   	= "audio_capture",
	.dma_addr	= CODEC_BASSADDRESS + SUNXI_ADC_RXDATA,//accept data address
};


static void read_dac_reg(void)
{
	unsigned reg_val;
#ifdef DEBUG
	DBG_MSG("###################################DAC REG DEBUG##############################\n");
	reg_val = readl(baseaddr + SUNXI_DAC_DPC);
	DBG_MSG("SUNXI_DAC_DPC 0x%X\n", reg_val);
	reg_val = readl(baseaddr + SUNXI_DAC_FIFOC);	
	DBG_MSG("SUNXI_DAC_FIFOC 0x%X\n", reg_val);
	reg_val = readl(baseaddr + SUNXI_DAC_FIFOS);	
	DBG_MSG("SUNXI_DAC_FIFOS 0x%X\n", reg_val);
	reg_val = readl(baseaddr + SUNXI_DAC_CNT);	
	DBG_MSG("SUNXI_DAC_CNT 0x%X\n", reg_val);	
	reg_val = readl(baseaddr + SUNXI_DAC_DEBUG);	
	DBG_MSG("SUNXI_DAC_DEBUG 0x%X\n", reg_val);
	reg_val = readl(baseaddr + SUNXI_DAC_DAP_CTR);	
	DBG_MSG("SUNXI_DAC_DAP_CTR 0x%X\n", reg_val);	
	reg_val = 0;
	reg_val = readb(baseaddr + LOMIXSC);	
	DBG_MSG("LOMIXSC 0x%X\n", reg_val);
	reg_val = readb(baseaddr + ROMIXSC);	
	DBG_MSG("ROMIXSC 0x%X\n", reg_val);		
	reg_val = readb(baseaddr + DAC_PA_SRC);	
	DBG_MSG("DAC_PA_SRC 0x%X\n", reg_val);	
	reg_val = readb(baseaddr + LINEOUT_VOLC);	
	DBG_MSG("LINEOUT_VOLC 0x%X\n", reg_val);
	reg_val = readb(baseaddr + MIC2_CTRL);	
	DBG_MSG("MIC2_CTRL 0x%X\n", reg_val);		
	reg_val = readb(baseaddr + DAC_PA_SRC);	
	DBG_MSG("DAC_PA_SRC 0x%X\n", reg_val);	
	DBG_MSG("###################################DAC REG DEBUG##############################\n");
#endif	
...
static void pa_power_en(int enable)
{
	long unsigned int config;
	if(enable){
		config = SUNXI_PINCFG_PACK(SUNXI_PINCFG_TYPE_FUNC,1);
		pin_config_set(SUNXI_PINCTRL,"PG10",config);
		config = SUNXI_PINCFG_PACK(SUNXI_PINCFG_TYPE_DRV,2);
		pin_config_set(SUNXI_PINCTRL,"PG10",config);
		config = SUNXI_PINCFG_PACK(SUNXI_PINCFG_TYPE_DAT,1);
		pin_config_set(SUNXI_PINCTRL,"PG10",config); 	
	}else{
		config = SUNXI_PINCFG_PACK(SUNXI_PINCFG_TYPE_FUNC,1);
		pin_config_set(SUNXI_PINCTRL,"PG10",config);
		config = SUNXI_PINCFG_PACK(SUNXI_PINCFG_TYPE_DRV,2);
		pin_config_set(SUNXI_PINCTRL,"PG10",config);
		config = SUNXI_PINCFG_PACK(SUNXI_PINCFG_TYPE_DAT,0);
		pin_config_set(SUNXI_PINCTRL,"PG10",config); 
	}
}
...
static void get_audio_param(void)
{
	...
	type = script_get_item("audio0", "codec_speaker_headset_en", &val);
	if (SCIRPT_ITEM_VALUE_TYPE_INT != type) {
        	pr_err("[audiocodec] codec_speaker_headset_en type err!\n");
    	} else {
		codec_speaker_headset_en = val.val;
	}	
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	DBG_MSG("codec_spekerout_en %d version_v3_used %d pa_double_used %d\n", 
		codec_speakerout_en,version_v3_used,pa_double_used);
...
int v3_codec_startup(struct snd_pcm_substream *substream,
	struct snd_soc_dai *dai)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct v3_pcm_dma_params *dma_data;	
	long unsigned int config;
	DBG_ENTER();	
...
void v3_codec_dev_shutdown(struct snd_pcm_substream *substream,
	struct snd_soc_dai *dai)
{
	int i = 0, cur_vol = 0;
	DBG_ENTER();
...
int v3_codec_prepare(struct snd_pcm_substream *substream,
	struct snd_soc_dai *dai)
{
	...
	DBG_ENTER();
	DBG_MSG("substream->stream %d SNDRV_PCM_STREAM_PLAYBACK %d \n", substream->stream,SNDRV_PCM_STREAM_PLAYBACK);
	DBG_MSG("substream->runtime->rate 0x%X\n", substream->runtime->rate);
	DBG_MSG("substream->runtime->channels 0x%X\n", substream->runtime->channels);
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
	...
	DBG_MSG("runtime->status->state %d SNDRV_PCM_STATE_XRUN %d\n", runtime->status->state,SNDRV_PCM_STATE_XRUN);
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		DBG_MSG("codec_speaker_headset_en %d \n", codec_speaker_headset_en);
		...
		if (runtime->status->state == SNDRV_PCM_STATE_XRUN) {
			DBG_MSG("[audio codec]capture xrun.\n");
		}
...
int v3_codec_trigger(struct snd_pcm_substream *substream,
...
int v3_codec_hw_params(struct snd_pcm_substream *substream,
	...
	struct snd_soc_dai *dai)
{
	...
	DBG_MSG("rtd->cpu_dai 0x%X, dai 0x%X\n", rtd->cpu_dai, dai);
	snd_soc_dai_set_dma_data(rtd->cpu_dai, substream, dma_data);
...
int v3_codec_set_dai_sysclk(struct snd_soc_dai *codec_dai,
...
int v3_codec_set_dai_fmt(struct snd_soc_dai *codec_dai,
			       unsigned int fmt)
{
	return 0;
}
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	DBG_MSG("codec_speaker_headset_en \n", codec_speaker_headset_en);
	if (codec_speaker_headset_en == 1) {
		ret = codec_pa_play_open();
	} else if (codec_speaker_headset_en == 0) {
		ret = codec_headphone_play_open();
...
static struct snd_soc_dai_ops v3_codec_dai_ops = {
...
static struct snd_soc_dai_driver v3_codec_dai = {
	.name = "v3-codec-recplay",
	/* playback capabilities */
	.playback = {
		.stream_name = "Playback",
		.channels_min = 1,
		.channels_max = 2,
		.rates = v3_codec_RATES,
		.formats = v3_codec_FORMATS,
	},
	.capture = {
		.stream_name = "Capture",
		.channels_min = 1,
	...
	/* pcm operations */
	.ops = &v3_codec_dai_ops,
};
EXPORT_SYMBOL(v3_codec_dai);

static int v3_codec_soc_probe(struct snd_soc_codec *codec)
{
	DBG_ENTER();	
	/* Add virtual switch */
...
static int v3_codec_soc_suspend(struct snd_soc_codec *codec)
{
...
static int v3_codec_soc_resume(struct snd_soc_codec *codec)
{
	DBG_ENTER();
...
/* power down chip */
static int v3_codec_soc_remove(struct snd_soc_codec *codec)
{
	DBG_ENTER();	
	return 0;
}

static struct snd_soc_codec_driver soc_codec_v3 = {
	.probe 	 =	v3_codec_soc_probe,
	.remove  =  v3_codec_soc_remove,
	.suspend = 	v3_codec_soc_suspend,
	.resume  =  v3_codec_soc_resume,
};
		...
		const char *buf, size_t count)
{
	...
	DBG_MSG("ret:%d, reg_group:%d, reg_offset:%d, reg_val:0x%x\n", ret, input_reg_group, input_reg_offset, input_reg_val);
...
static int __init v3_codec_probe(struct platform_device *pdev)
{
	int err = -1;
	int req_status;
	DBG_ENTER();	
	...
	snd_soc_register_codec(&pdev->dev, &soc_codec_v3, &v3_codec_dai, 1);
...
static int __exit v3_codec_remove(struct platform_device *pdev)
{
	DBG_ENTER();	
	if ((NULL == codec_moduleclk)||(IS_ERR(codec_moduleclk))) {
		pr_err("codec_moduleclk handle is invaled, just return\n");
...
static void v3_codec_shutdown(struct platform_device *devptr)
{
	DBG_ENTER();
	if (pa_gpio_config) {
		gpio_set_value(item.gpio.gpio, 0);
...
/*data relating*/
static struct platform_device v3_codec_device = {
	.name = "v3-codec",
	.id = -1,
};

/*method relating*/
static struct platform_driver v3_codec_driver = {
	.driver = {
		.name = "v3-codec",
		.owner = THIS_MODULE,
	},
	.probe = v3_codec_probe,
	.remove = __exit_p(v3_codec_remove),
	.shutdown = v3_codec_shutdown,
};

static int __init v3_codec_init(void)
{
	int err = 0;
	DBG_ENTER();
	if((err = platform_device_register(&v3_codec_device)) < 0)
		return err;

	if ((err = platform_driver_register(&v3_codec_driver)) < 0)
		return err;
	DBG_EXIT();
	return 0;
}
module_init(v3_codec_init);

static void __exit v3_codec_exit(void)
{
	DBG_ENTER();
	platform_driver_unregister(&v3_codec_driver);
	DBG_EXIT();
}
module_exit(v3_codec_exit);

MODULE_DESCRIPTION("V3 codec driver");
MODULE_LICENSE("GPL");
MODULE_ALIAS("platform:v3-codec");
